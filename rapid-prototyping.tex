\documentclass[aspectratio=169]{beamer}
\usetheme[style=noir]{fhtw}
\usepackage{preamble}

\title[Rapid Prototyping]{Rapid Prototyping}
\subtitle{CI - ASSIST HEIDI}
\author{Alija Sabic, \glsentrytext{msc}}
\mail{sabic@technikum-wien.at}
\institute{Department Electronic Engineering}

\NewDocumentCommand{\res}{ m }{``\textbf{\usebibentry{#1}{title}}'', \usebibentry{#1}{url} \cite{#1}}
% \NewDocumentCommand{\res}{ m }{\bibentry{#1} \cite{#1}} % only supported by bibtex

\setlength{\parskip}{1em}

\begin{document}

\begin{frame}[plain]
    \titlepage
    \nocite{aigner:rapid-prototyping}
    \nocite{fasterman:2012}
    \nocite{benchoff:2014:1}
    \nocite{benchoff:2014:2}
    \nocite{hubs}
\end{frame}

\section{Literature}

\begin{frame}
    \begin{itemize}
        \item Slides are based on Benjamin Aigner, \acs{msc}: \res{aigner:rapid-prototyping}
        \item \res{fasterman:2012}
        \item \res{benchoff:2014:1}
        \item \res{benchoff:2014:2}
        \item \res{hubs}
    \end{itemize}
\end{frame}

\section{Rapid Prototyping}

\input{rp/definition}
\subsection{Technologies}
\input{rp/technologies}
\subsection{\glsentrydesc{fdm}}
\input{rp/additive/fdm-poster}
\subsection{FabLabs}
\input{rp/fablabs}
\subsection{Social Impact}
\input{rp/social-impact}

\section{3D Printing}

\subsection{\glsentrydesc{fdm}}
\input{rp/additive/fdm}
\subsection{\glsentrydesc{dlp}}
\input{rp/additive/dlp}
\subsection{\glsentrydesc{sls}}
\input{rp/additive/sls}
\subsection{Materials}
\input{rp/additive/materials}
\subsection{Workflow}
\input{rp/additive/workflow}
\subsection{\glsentrytext{stl} Files}
\input{rp/stl}
\subsection{Slicing}
\input{rp/slicing}
\subsection{G-code}
\input{rp/gcode}
\subsection{Software and Platforms}
\subsubsection{Thingiverse}
\input{rp/thingiverse}
\subsubsection{\glsentrytext{cad}}
\input{rp/cad}

\section{Laser Cutter}

\subsection{Technology}
\input{rp/subtractive/laser-cutter/overview}
\subsection{Laser}
\input{rp/subtractive/laser-cutter/laser}
\subsection{Advantages}
\input{rp/subtractive/laser-cutter/advantages}
\subsection{Disadvantages}
\input{rp/subtractive/laser-cutter/disadvantages}
\subsection{Products}
\input{rp/subtractive/laser-cutter/products}
\subsection{Examples}
\input{rp/subtractive/laser-cutter/examples}


\section{Other Technologies}

\subsection{Lego \texttrademark}
\input{rp/other/lego}
\subsection{Plaast/Instamorph}
\input{rp/other/plaast}
\subsection{\glsentrytext{pvc} Pipes}
\input{rp/other/pvc-pipes}
\subsection{LocLine}
\input{rp/other/locline}

\appendix

\begin{frame}[allowframebreaks]{Acronyms}
    \glsadd{pvc}
    \glsadd{sls}
    \printglossary[type=\acronymtype, nonumberlist]
\end{frame}

\begin{frame}[label=references]{References}
    \bibliography{references}
\end{frame}

\end{document}